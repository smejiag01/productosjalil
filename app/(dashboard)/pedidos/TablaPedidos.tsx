    ...
    <div className="py-16 text-center text-gray-400">
      <div className="flex flex-col items-center gap-2">
        ...
        <p className="text-xs text-gray-300">
          Los pedidos aparecerán aquí cuando los clientes los realicen por WhatsApp
        </p>
      </div>
    </div>
  );
        ...
        {FILTROS.map((f) => {
          ...
          return (
            <button
              ...
              className={`px-3 sm:px-4 py-2 rounded-full text-sm font-medium transition-colors flex items-center gap-1.5 whitespace-nowrap flex-shrink-0 ${
                activo
                  ? "bg-brand text-white"
              ...
              <div className="flex items-start justify-between mb-2">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400">{pedido.rutaNombre}</p>
                  </div>
                </div>
              ...
              pedidosFiltrados.map((pedido) => (
                <tr key={pedido.id} className="hover:bg-gray-50/50 transition-colors">
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 bg-gray-100 rounded-full flex items-center justify-center text-xs font-semibold text-gray-600">
                        {pedido.clienteNombre.split(" ").map((p) => p[0]).join("").substring(0, 2).toUpperCase()}
                      </div>
                      <div>
                        <p className="text-sm font-medium text-gray-900">{pedido.clienteNombre}</p>
                        {pedido.clienteCodigo && <p className="text-xs text-gray-400">{pedido.clienteCodigo}</p>}
                      </div>
                    </div>
                  </td>
                  <td className="py-3 px-4 text-sm text-gray-600">{pedido.rutaNombre}</td>
                  <td className="py-3 px-4 text-sm text-gray-600">{pedido.numProductos} productos</td>
                  <td className="py-3 px-4 text-sm font-medium text-gray-900">{pedido.total}</td>
                  <td className="py-3 px-4"><BadgeEstado estado={pedido.estado} /></td>
                  <td className="py-3 px-4 text-right">
                    <Link href={`/pedidos/${pedido.id}`} className="text-sm text-brand hover:text-brand-light font-medium transition-colors">Ver detalle</Link>
                  </td>
                </tr>
              ))
            )}
