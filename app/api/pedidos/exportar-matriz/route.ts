...
import { libroMatrizPorRuta } from "@/lib/exportaciones/excel";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const buffer = libroMatrizPorRuta(pedidos, fecha);

    return new NextResponse(new Uint8Array(buffer), {
      headers: {
        "Content-Type":
          "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        "Content-Disposition": `attachment; filename="matriz-pedidos-${fecha}.xlsx"`,
      },
    });
  } catch (error) {
    console.error("Error al exportar matriz de pedidos:", error);
